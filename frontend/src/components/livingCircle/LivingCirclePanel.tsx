  ...
  return (
    <section className="living-circle-panel" aria-label="步行十五分钟生活圈分析">
      ...
      <div className="living-origin-row">
        <span>分析中心</span>
        <b>{origin ? `${origin.lon.toFixed(5)}, ${origin.lat.toFixed(5)}` : '待选择'}</b>
      </div>
      ...
      {result && status === 'success' && (
        <>
          ...
          <div className="living-model">
            ...
            <span>起点吸附</span><strong>{result.origin_snap.snap_distance_m.toFixed(0)} m</strong>
            <span>15分钟可达 POI</span><strong>{result.summary.reachable_poi_count}</strong>
          </div>
          ...
          <div className="living-category-stats">
            {result.categories.map((item) => (
              <div key={item.category} className={!selectedCategories.includes(item.category) ? 'is-muted' : ''}>
                ...
                <small>{item.unique_subcategory_count} 子类 · 最近 {item.nearest_poi?.name || '—'} {item.nearest_walk_time_min === null ? '' : `${item.nearest_walk_time_min.toFixed(1)} min`}</small>
              </div>
            ))}
          </div>
          ...
          <p className="living-disclaimer">结果基于静态步速与 OSM 步行网络，覆盖率不代表主观评价或实时路况。</p>
        </>
      )}
