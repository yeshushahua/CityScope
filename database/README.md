...
Phase 9 的 `007_phase9_building_heights.sql` 增加 OSM 建筑高度、楼层数、展示高度及来源字段。`scripts/osm/enrich_building_heights.py` 从已缓存的真实 OSM 响应恢复 `height` / `building:levels` 标签；缺少两个标签的建筑保持未知高度。
